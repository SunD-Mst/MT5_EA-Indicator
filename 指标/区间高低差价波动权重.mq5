...
input ENUM_TIMEFRAMES InputPeriod    = PERIOD_M5;       // 计算区间和所用的K线周期
input int            HistoryDays     = 14;              // 历史天数（计算均值）
...
//+------------------------------------------------------------------+
//| 计算指定时段内的K线高低差总和 (Range Sum)                           |
//+------------------------------------------------------------------+
bool CalculateRangeSum(datetime segStart, datetime segEnd, double &rangeSum)
{
   double high[], low[];
   // 复制时段内的高、低价序列
   int copiedHigh = CopyHigh(Symbol(), InputPeriod, segStart, segEnd, high);
   int copiedLow  = CopyLow(Symbol(), InputPeriod, segStart, segEnd, low);
   
   if(copiedHigh < 1 || copiedLow < 1 || copiedHigh != copiedLow)
   {
      Print("时段内K线数据复制失败或数量不一致: High=", copiedHigh, " Low=", copiedLow);
      return false;
   }
   
   double sum = 0.0;
   for(int i = 0; i < copiedHigh; i++)
   {
      sum += (high[i] - low[i]);
   }
...
//+------------------------------------------------------------------+
//| 收集指定日期之前的历史时段区间和序列（用于计算该日期的统计量）        |
//+------------------------------------------------------------------+
int CollectHistoricalRangeSums(datetime baseDayStart, double &rangeArray[])
{
   ArrayResize(rangeArray, 0);
   ...
   while(collected < HistoryDays && attempts < maxAttempts)
   {
      ...
      {
         double rangeSum;
         if(CalculateRangeSum(segStart, segEnd, rangeSum))
         {
            ArrayResize(rangeArray, collected+1);
            rangeArray[collected] = rangeSum;
            collected++;
         }
      }
...
bool CalculateWeightForDate(datetime dayStart, double &weight, datetime &effectiveTime)
{
   ...
   // 1. 收集该日期之前的历史区间和序列
   double histRanges[];
   int histCount = CollectHistoricalRangeSums(dayStart, histRanges);
   if(histCount < 1)
   {
      Print("日期 ", TimeToString(dayStart), " 有效历史交易日不足1天，跳过");
      return false;
   }
   ...
   for(int i = 0; i < histCount; i++)
      sum += histRanges[i];
   double avgRange = sum / histCount;
   
   // 3. 计算该日期的区间和
   double rangeToday;
   if(!CalculateRangeSum(segStart, segEnd, rangeToday))
   {
      Print("日期 ", TimeToString(dayStart), " 计算当日区间和失败");
      return false;
   }
...
void UpdateTodayWeight()
{
   ...
   if(CalculateWeightForDate(todayStart, weight, effTime))
   {
      ...
      // 获取详细信息用于显示
      double histRanges[];
      int histCount = CollectHistoricalRangeSums(todayStart, histRanges);
      double avgRange = 0.0;
      if(histCount > 0)
      {
         double sumHist = 0.0;
         for(int i = 0; i < histCount; i++)
            sumHist += histRanges[i];
         avgRange = sumHist / histCount;
      }
      double rangeToday = 0.0;
      CalculateRangeSum(segStartToday, segEndToday, rangeToday);
